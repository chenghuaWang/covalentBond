...
// void http_process(WFHttpTask* server_task, const char* root) {
...
//   // std::cout << "exname: "<< file_name_extension << std::endl;
...
cb::utils::cbWebserver::cbWebserver(const int32_t& port, const std::string& root)
    : m_port(port), m_root(root) {
  m_server = new WFHttpServer([=](WFHttpTask* server_task) {
    protocol::HttpRequest* req = server_task->get_req();
    protocol::HttpResponse* resp = server_task->get_resp();
